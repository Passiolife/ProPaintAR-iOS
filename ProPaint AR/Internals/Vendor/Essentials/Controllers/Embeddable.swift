...
extension UIViewController {
    ...
    public func embed<T>(controller vc: T, into parentView: UIView?, layout: LayoutBlock = { v, pv in
    ...
    where T: UIViewController {
        guard let container = parentView ?? self.view
        else { return }
    ...
    where T: Presentable {
        guard let vcView = vc.toPresentable().view,
              let container = parentView ?? self.view
        else { return }
        
        addChild(vc.toPresentable())
        container.addSubview(vcView)
        vcView.translatesAutoresizingMaskIntoConstraints = false
        layout(vcView, container)
        vc.toPresentable().didMove(toParent: self)
    ...
    public func unembed(controller: Presentable) {
        let vc = controller.toPresentable()
...
extension Coordinator {
    ...
    where T: UIViewController {
        guard let container = parentView ?? toPresentable().view
        else { return }
        
        toPresentable().addChild(vc)
        container.addSubview(vc.view)
        vc.view.translatesAutoresizingMaskIntoConstraints = false
        layout(vc.view, container)
        vc.didMove(toParent: toPresentable())
    ...
    where T: Presentable {
        guard let vcView = vc.toPresentable().view,
              let container = parentView ?? toPresentable().view
        else { return }
        
        toPresentable().addChild(vc.toPresentable())
        container.addSubview(vcView)
        vcView.translatesAutoresizingMaskIntoConstraints = false
        layout(vcView, container)
        vc.toPresentable().didMove(toParent: toPresentable())
