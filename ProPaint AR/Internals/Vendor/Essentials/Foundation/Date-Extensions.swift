...
extension Date {
	...
	public func weekday() -> Int {
		let calendar = Calendar.current
		let components = calendar.dateComponents([Calendar.Component.weekday], from: self)
	...
	public func daysFrom(date: Date) -> Int {
        ...
        let dateComponents: Set<Calendar.Component> = [Calendar.Component.day]
        
        guard let day = calendar.dateComponents(dateComponents, from: earliest, to: latest).day
        else { return 0 }
	...
	public func minutesFrom(date: Date) -> Int {
		...
		let latest = (earliest == self) ? date : self
		let multiplier = (earliest == self) ? -1 : 1
        let dateComponents: Set<Calendar.Component> = [Calendar.Component.minute]
        
        guard let minute = calendar.dateComponents(dateComponents,
                                                   from: earliest,
                                                   to: latest).minute
        else { return 0 }
