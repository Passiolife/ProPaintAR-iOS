...
open class FSPageControl: UIControl {
    ...
    @IBInspectable
    open var contentInsets: CGFloat = 0 {
        didSet {
            self.setNeedsLayout()
        }
    }
    ...
    override open func layoutSubviews() {
        ...
        self.contentView.frame = {
            let x = self.contentInsets
            let y = self.contentInsets
            let width = self.frame.width - 2 * self.contentInsets
            let height = self.frame.height - 2 * self.contentInsets
    ...
    override open func layoutSublayers(of layer: CALayer) {
        ...
        var x: CGFloat = {
            ...
            case .center, .fill:
                ...
                let part1 = CGFloat(self.numberOfPages / 2) * diameter
                let part2 = spacing * CGFloat((self.numberOfPages - 1) / 2)
                let amplitude = part1 + part2
                return midX - amplitude
                
            case .right, .trailing:
                let part1 = diameter * CGFloat(self.numberOfPages)
                let part2 = CGFloat(self.numberOfPages - 1) * spacing
                let contentWidth = part1 + part2
    ...
    fileprivate func updateIndicatorAttributes(for layer: CAShapeLayer) {
        ...
        if let image = self.images[state] {
            ...
            let alternatePath = UIBezierPath(
                ovalIn: CGRect(x: 0,
                               y: 0,
                               width: self.itemSpacing,
                               height: self.itemSpacing)
            ).cgPath
            layer.path = self.paths[state]?.cgPath ?? alternatePath
        }
...
extension UIControl.State: Hashable {
    public func hash(into hasher: inout Hasher) {
        hasher.combine(Int((6777 * rawValue + 3777) % UInt(UInt16.max)))
    }
}
